...
export default function AdminPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, key: string, targetIndex?: number) => {
    showStatus('error', 'Image uploads have been moved to /public directory for static serving. Please commit new images to git.')
  }
        ...
        <aside className="w-full md:w-64 shrink-0 flex flex-col gap-2">
          {TABS.map(({ id, label, Icon }) => (
            <button
              ...
              className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl font-bold text-xs transition-all w-full text-left cursor-pointer ${
                activeTab === id
                  ? 'bg-[rgba(255,107,43,0.12)] text-[var(--primary)]'
